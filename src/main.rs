...
fn normalize_one(input_sdat_path: &Path, sdat_keys: &SdatKeys) -> Result<()> {
    ...
    let mut extracted = extract_archive_to_dir(&archive_bytes, archive_kind, endianness, &extract_root)
    ...
    let txt = sharc_timestamp.to_be_bytes()
        .iter()
        .map(|b| format!("{:02X}", b))
        .collect::<Vec<_>>()
        .join("");
...
fn repack_to_sharc(extracted: &mut [ExtractedEntry], endianness: Endianness) -> Result<(Vec<u8>, i32)> {
